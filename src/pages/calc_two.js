import React from "react";
import { connect } from "react-redux";
import { compose } from "redux";
import { Link } from "react-router-dom";
import { firebaseConnect, isLoaded, isEmpty } from "react-redux-firebase";

/* material ui stuff*/
import TextField from "@material-ui/core/TextField";
import FormControl from "@material-ui/core/FormControl";
import Button from "@material-ui/core/Button";
import AddIcon from "@material-ui/icons/Add";
import Paper from "@material-ui/core/Paper";
import Grid from "@material-ui/core/Grid";
import { withStyles } from "@material-ui/core/styles";
import Chart from "react-google-charts";
import {Redirect} from 'react-router';
import Header from "../components/header.js";

const pieOptions = {
    title: 'My Daily Activities',
	slices: [
    {
      color: "#2c3646"
    },
    {
      color: "#688FC2"
    },
    {
      color: "#0F2DD2"
    },
    {
      color: "#72BAEA"
    },
    {
      color: "#5EC6D2"
    },
    {
      color: "#00525B"
    },
    {
      color: "#1D2889"
    },
    {
      color: "#D6DBDF"
    },
    {
      color: "#EBEDEF"
    }
  ],
  legend: {
    position: "bottom",
    alignment: "center",
    textStyle: {
      color: "#233238",
      fontSize: 14
    }
  },
  tooltip: {
    showColorCode: true
  },
  chartArea: {
    left: 0,
    top: 0,
    width: "100%",
    height: "80%",
    paddingTop:0
  },
  backgroundColor: '#d7e1e5',
      is3D: true,
};
class CalcTwoPage extends React.Component {
	constructor(props) {
		super(props);
		this.state = {
			Expenses: [
				{
					Name: "",
					Cost: 0,
				},
			],
			done: false
		};

	}

	handleChange(event, field) {
		this.setState({
			[field]: Number.parseInt(event.target.value)
		});
	}

	navigateAway(event){
		event.preventDefault();
		let uid = this.props.auth.uid;
	    this.props.firebase.update(`/users/${uid}/calc_two`, this.state.Expenses)
   		.then((response) => {
      		this.setState({
       			done: true
        	});
  		})
  	}



	addRow(){
		this.setState({
			Expenses: [...this.state.Expenses,
				{
					Name: "",
					Cost: 0,
				}
			]
		});
	}

	handleExpenseChange(event, fieldName, index){
		let Expenses = this.state.Expenses;
		let value;
		if(fieldName == "Cost"){
			value = Number.parseInt(event.target.value);
		}else{
			value = event.target.value;
		}
		Expenses[index][fieldName] = value;
		this.setState({Expenses: Expenses});
	}

	render() {
		let payload;
		if (!isLoaded(this.props.calc_two)) {
			payload = null;
		}
		if (isLoaded(this.props.calc_two) && !isEmpty(this.props.calc_two)) {
			payload = Object.keys(this.props.calc_two).map(key => {
				let calc_two = this.props.calc_two[key];
				const styles = theme => ({
					root: {
						flexGrow: 1
					}
				});
			});
		}
		if(this.state.done){
  	  return(<Redirect to='calc_three' />);
  	}
		let allExpenses = this.state.Expenses.map((Expense, index) => {
			return (
				<Grid container spacing={24}>
				<br /><br />
					<Grid item xs={3} id="gridItem">
						<FormControl fullWidth>
							<TextField label="Expense Name" margin="normal"
							onChange={(event) => {this.handleExpenseChange(event, 'Name', index)}}
							/>
						</FormControl>
					</Grid>

					<Grid item xs={3} id="gridItem">
						<FormControl fullWidth>
							<TextField label="Cost" margin="normal"
							onChange={(event) => {this.handleExpenseChange(event, 'Cost', index)}}
							/>
						</FormControl>
					</Grid>
				</Grid>
			);
		});
		var chartexpenses=[["Monthly Expenses", "Cost"]];
		for (var i=0; i <this.state.Expenses.length; i++){
			chartexpenses.push([this.state.Expenses[i].Name,this.state.Expenses[i].Cost])
		}
		
			  		
		if(!isLoaded(this.props.profile)){
	// not ready to be ready
	//LOADING PAGE
}

if(isLoaded(this.props.profile) && isEmpty(this.props.profile)){
	// assume user is not logged in
	//MESSAGE THAT SAYS USER MUST LOG IN TO SEE CALCULATOR
}
console.log(this.props.profile.calc_one);
if(isLoaded(this.props.profile) && !isEmpty(this.props.profile) && typeof this.props.profile.calc_one !== "undefined"){
var totalExpenses = this.props.profile.calc_one.Monthly_Salary;
		for (var i=0; i<this.state.Expenses.length; i++){
			totalExpenses -= this.state.Expenses[i].Cost;
		}
	chartexpenses.push(["Remaining Income",totalExpenses]);
			payload = <div>

		 <button class="numCss">
	           		{this.props.profile.name}'s Monthly Salary: {this.props.profile.calc_one.Monthly_Salary}
			  </button>

			  		
			          <button class="numCss1">

			            {this.props.profile.name}'s Income After Expenses: {totalExpenses}
			    </button>
			    </div>
}
		return (
			<div>

        <Header />
          <br />
    			<br />
    			<br />
    			<br />
					<div class="linecss" id="calcs">
			        <Link to="/calc_one">
			          <button class="buttonCss1">1</button>
			        </Link>
			        <Link to="/calc_two">
			          <button class="buttonCss1">2</button>
			        </Link>
			        <Link to="/calc_three">
			          <button class="buttonCss">3</button>
			        </Link>
			        <Link to="/calc_four">
			          <button class="buttonCss">4</button>
			        </Link>
			        <Link to="/calc_five">
			          <button class="buttonCss">5</button>
			        </Link>
					</div>
			        <br />
			        <br />
			        <br />
			    
					<font size="20">What are your monthly essential expenses?</font>
					
<br />
<br />
			          {payload}
				<form onSubmit={event => {this.navigateAway(event)}}>
								<div className="CalcThreePage">
			        <Chart
			        style={{display: isLoaded(this.props.profile) && !isEmpty(this.props.profile) ? 'block' : 'none'}}
			          chartType="PieChart"
			          data={
			          		chartexpenses
			          }
			          graph_id="PieChart"
			          options={pieOptions}
			          width={"100%"}
			          height={"400px"}
			        />
		    	  </div>
					{allExpenses}

					<Button
						variant="fab"
						mini
						color="secondary"
						aria-label="Add"
						id="addButton"
						onClick={() => {
							this.addRow();
						}}
					>
						<AddIcon />

					</Button>
					<br />
					<br />
					<br />
					<br />
					<br />
					<br />
					<Link to="/calc_one">
						<Button
							variant="contained"
							color="primary"
						>
							Back
						</Button>
					</Link>


					<Button
						type="submit"
						variant="contained"
						color="primary"

					>
						Next
						</Button>
				</form>
				<br />
				<br />

				{/* <ul>
    		{payload}
    		</ul> */}
			</div>
		);
	}
}

export default compose(
	firebaseConnect(props => [{ path: "calc_two" }]),
	connect(({firebase: {auth}}) => ({auth})),
	connect((state, props) => ({
		calc_two: state.firebase.data.calc_two
	})),
	connect(({firebase: {profile}}) => ({profile}))
)(CalcTwoPage);
